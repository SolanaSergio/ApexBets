...
class SportAPIClient {
  ...
  private rateLimitTracker: Map<
    string,
    { lastRequest: number; requestCount: number; failures: number }
  > = new Map()
  private circuitBreaker: Map<string, { failures: number; lastFailure: number; isOpen: boolean }> =
    new Map()
  private performanceMetrics: Map<
    string,
    { avgResponseTime: number; successRate: number; lastUpdate: number }
  > = new Map()
  ...
  async fetchFromAPI(
    ...
    const breaker = this.circuitBreaker.get(configKey) || {
      failures: 0,
      lastFailure: 0,
      isOpen: false,
    }
    if (breaker.isOpen && now - breaker.lastFailure < 300000) {
      // 5 minute circuit breaker
      throw new Error(`Circuit breaker open for ${configKey}. Too many failures.`)
    }

    // Rate limiting per API with dynamic adjustment
    const tracker = this.rateLimitTracker.get(configKey) || {
      lastRequest: 0,
      requestCount: 0,
      failures: 0,
    }
      ...
      if (!response.ok) {
        ...
        if (response.status >= 500) {
          ...
          if (tracker.failures >= 5) {
            this.circuitBreaker.set(configKey, {
  ...
  private updatePerformanceMetrics(
    ...
    const current = this.performanceMetrics.get(configKey) || {
      avgResponseTime: 0,
      successRate: 1,
      lastUpdate: 0,
    }
    ...
    // Update success rate
    const totalRequests =
      current.successRate === 1 ? 1 : Math.max(1, (now - current.lastUpdate) / 1000)
  ...
  private validateRealTimeData(
    ...
    if (endpoint.includes('games') || endpoint.includes('scoreboard')) {
      const gamesArr = Array.isArray(data?.games)
        ? data.games
        : Array.isArray(data?.data)
          ? data.data
          : []
      if (Array.isArray(gamesArr)) {
        const liveGames = gamesArr.filter(
          (game: any) =>
            game?.status === 'live' ||
  ...
  private async addProviderSpecificParams(
    url: URL,
    config: APIConfig,
    _endpoint: string
  ): Promise<void> {
      ...
      case 'nhl-stats':
        if (Array.isArray(data?.dates)) {
          return {
            ...
            count: data.dates.reduce(
              (total: number, date: any) =>
                total + (Array.isArray(date?.games) ? date.games.length : 0),
              0
            ),
          }
...
async function syncGames(
  ...
  try {
    ...
    const gamesToInsert = gamesData
      .map((game: Record<string, unknown>) => normalizeGameData(game, sportConfig, apiConfig))
      .filter(Boolean)

    if (gamesToInsert.length === 0) return 0

    // Upsert games
    const { error } = await supabase.from('games').upsert(gamesToInsert, {
      onConflict: 'id',
...
async function syncTeams(
  ...
  try {
    ...
    const teamsToInsert = teamsData
      .map((team: Record<string, unknown>) => normalizeTeamData(team, sportConfig, apiConfig))
      .filter(Boolean)
...
async function syncPlayers(
  ...
  try {
    ...
    const playersToInsert = playersData
      .map((player: Record<string, unknown>) => normalizePlayerData(player, sportConfig, apiConfig))
      .filter(Boolean)

    if (playersToInsert.length === 0) return 0

    // Upsert players
    const { error } = await supabase.from('player_profiles').upsert(playersToInsert, {
      onConflict: 'id',
      ignoreDuplicates: false,
    })
...
async function syncStandings(
  ...
  try {
    ...
    const standingsToInsert = standingsData
      .map((standing: Record<string, unknown>, index: number) =>
    ...
    // Upsert standings
    const { error } = await supabase.from('league_standings').upsert(standingsToInsert, {
      onConflict: 'id',
      ignoreDuplicates: false,
    })
...
function normalizeGameData(
  ...
  return {
    ...
    away_team_id:
      game?.away_team?.id || game?.teams?.away?.team?.id || game?.VISITOR_TEAM_ID || null,
    home_team_name:
      game?.home_team?.name || game?.teams?.home?.team?.name || game?.HOME_TEAM_NAME || null,
    away_team_name:
      game?.away_team?.name || game?.teams?.away?.team?.name || game?.VISITOR_TEAM_NAME || null,
    game_date:
      game?.date ||
      game?.gameDate ||
      game?.fixture?.date ||
    ...
    status: normalizeStatus(
      game?.status || game?.fixture?.status?.short || game?.GAME_STATUS_TEXT || 'scheduled'
    ),
    home_score:
      game?.home_score ??
    ...
    away_score:
      ...
      game?.goals?.away ??
      game?.VISITOR_TEAM_WINS ??
      null,
    venue:
      game?.venue?.name || game?.fixture?.venue?.name || game?.ARENA_NAME || game?.venue || null,
...
function normalizePlayerData(
  ...
  return {
    id: uniqueId,
    name:
      player?.name || player?.fullName || player?.displayName || player?.DISPLAY_FIRST_LAST || null,
    sport: sportConfig.name,
    position:
      player?.position || player?.primaryPosition?.name || player?.POSITION || player?.pos || null,
    team_id: player?.team_id || player?.team?.id || player?.TEAM_ID || player?.teamId || null,
    team_name:
      player?.team?.name || player?.team_name || player?.TEAM_NAME || player?.teamName || null,
    ...
    country:
      player?.country || player?.COUNTRY || player?.birthCountry || player?.nationality || null,
    jersey_number:
...
function normalizeStandingsData(
  standing: any,
  sportConfig: SportConfig,
  apiConfig: APIConfig,
  index: number
  ...
  // Generate unique ID based on sport and provider
  const teamId =
    standing.team?.id || standing.team_id || standing.TEAM_ID || standing.teamId || index
  ...
  return {
    ...
    season: sportConfig.current_season || getCurrentSeason(sportConfig),
    team_id:
      standing?.team?.id || standing?.team_id || standing?.TEAM_ID || standing?.teamId || null,
    team_name:
      standing?.team?.name ||
      standing?.team_name ||
      standing?.TEAM_NAME ||
  ...
  try {
    ...
    for (const sportConfig of sportsToSync) {
      ...
      try {
        ...
        const sortedProviders = apiProviders
          .map((provider: string) => ({
            provider,
            score:
              sportAPIClient.getOptimalProvider(sportConfig.name, 'games') === provider ? 1 : 0.5,
          }))
        ...
        for (const { provider } of sortedProviders) {
          ...
          if (!apiConfig) {
            console.warn(
              `No API configuration found for ${sportConfig.name} with provider ${provider}`
            )
            continue
          }
