          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
                <div className={`w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 bg-gradient-to-br ${item.gradient} rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform ${item.glowClass}`}>
                  <IconComponent className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-white" />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-bold text-lg sm:text-xl text-slate-800 truncate">{item.title}</p>
                  <p className="text-slate-600 font-medium text-sm sm:text-base truncate">{item.subtitle}</p>
                </div>
              </div>
              <div className="text-2xl sm:text-3xl font-black text-gradient mb-2 break-all">{item.value}</div>
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 bg-green-500 rounded-sm animate-pulse flex-shrink-0"></div>
                <span className={`${item.statusColor} font-bold text-xs sm:text-sm truncate`}>{item.status}</span>
              </div>
            </div>
          </div>
        )
