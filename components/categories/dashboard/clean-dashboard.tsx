...
export function CleanDashboard({ className = '', defaultSport = null }: CleanDashboardProps) {
    ...
    try {
      ...
      try {
        ...
        if (games.length === 0) {
          ...
          console.log(
            `External API: Loaded ${games.length} upcoming games for ${sport} on ${tomorrow}`
          )
        }
        ...
        if (games.length === 0) {
          ...
          console.log(
            `External API (no date filter): Loaded ${allScheduledGames.length} scheduled games for ${sport}`
          )
  ...
  const loadLiveOdds = useCallback(async (sport: SupportedSport) => {
    try {
      ...
      const validOdds = Array.isArray(odds)
        ? odds.filter(odd => odd && odd.home_team && odd.away_team)
        : []
  ...
  const loadDataForSport = useCallback(
    async (sport: SupportedSport, retryCount = 0) => {
      try {
        setLoading(true)
    ...
    [
      loadLiveGamesForSport,
      loadLiveOdds,
      loadServiceHealth,
      loadStandings,
      loadTeamsForSport,
      loadUpcomingGamesForSport,
  ...
  const isServiceHealthy = selectedSupportedSport
    ? (serviceHealth[selectedSupportedSport] ?? false)
    : false
          ...
          <div className="flex flex-col space-y-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground">Multi-sport analytics and predictions</p>
              </div>
          ...
          >
            <Card className="h-full">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">League standings</p>
              </CardContent>
            </Card>
          </StaggerItem>
          ...
          >
            <Card className="h-full">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">Upcoming predictions</p>
              </CardContent>
            </Card>
          </StaggerItem>
                ...
                <ScaleIn delay={100}>
                  <Card className="transition-all duration-300 hover:shadow-lg">
                    <CardHeader className="flex flex-col items-start pb-4">
                      ...
                      <CardDescription>Currently playing across all sports</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        {allLiveGames.length > 0 ? (
                          allLiveGames.slice(0, 5).map(game => (
                            <div
                              key={game.id}
                              className="flex items-center justify-between p-4 rounded-lg border-2 border-green-200 bg-gradient-to-r from-green-50 to-emerald-50 hover:shadow-md transition-all"
                            >
                ...
                <ScaleIn delay={200}>
                  <Card className="transition-all duration-300 hover:shadow-lg">
                    <CardHeader className="flex flex-col items-start pb-4">
                      ...
                      <CardDescription>Today's scheduled games</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        {allUpcomingGames.length > 0 ? (
                          allUpcomingGames.slice(0, 5).map(game => (
                            <div
                              key={game.id}
                              className="flex items-center justify-between p-4 rounded-lg border border-blue-200 bg-gradient-to-r from-blue-50 to-sky-50 hover:shadow-md transition-all"
                            >
                              <div className="flex items-center space-x-4">
                                <div className="flex items-center space-x-2">
                                  <TeamLogo
