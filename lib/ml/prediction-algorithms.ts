...
export class LogisticRegressionModel {
  ...
  static predict(homeStats: TeamStats, awayStats: TeamStats, context: GameContext): MLPrediction {
    ...
    const logit = featureDiff.reduce(
      (sum, feature, i) => sum + feature * weights[i],
      weights[weights.length - 1] // bias term
    )
  ...
  private static extractFeatures(
    ...
    return [
      ...
      totalGames > 0
        ? Math.max(-1, Math.min(1, (stats.pointsFor - stats.pointsAgainst) / totalGames / 20))
        : 0,
  ...
  private static calculateConfidence(
    ...
    const avgFeatureDiff = Math.abs(
      featureDiff.reduce((sum, diff) => sum + Math.abs(diff), 0) / featureDiff.length
    )
  ...
  private static predictSpread(
    ...
    const homeAvgMargin =
      homeGames > 0 ? (homeStats.pointsFor - homeStats.pointsAgainst) / homeGames : 0
    const awayAvgMargin =
      awayGames > 0 ? (awayStats.pointsFor - awayStats.pointsAgainst) / awayGames : 0
  ...
  private static generateFactors(
    featureDiff: number[],
    _homeStats: TeamStats,
    _awayStats: TeamStats
  ): string[] {
    ...
    if (Math.abs(featureDiff[5]) > 0.3) {
      factors.push(
        `Point differential ${featureDiff[5] > 0 ? 'favors home team' : 'favors away team'}`
      )
    }
