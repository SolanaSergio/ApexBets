...
export class PredictionTracker {
  ...
  static async evaluatePrediction(
    ...
    try {
      ...
      const evaluation: PredictionEvaluation = {
        ...
        predictedOutcome: typeof predictedOutcome === 'number' ? predictedOutcome : String(predictedOutcome),
        probability,
        isCorrect,
        error,
        calibrationBin,
  ...
  static async getModelPerformance(
    ...
    try {
      ...
      const byConfidenceLevel = this.calculateMetricsByConfidence(predictions, sport, league || 'all')
   ...
   */
  static async getCalibrationAnalysis(sport: string, modelName?: string): Promise<{
    ...
    try {
      ...
      // Generate recommendations
      const recommendations = this.generateCalibrationRecommendations(calibrationScore, calibrationCurve)
  ...
  static async monitorLivePerformance(): Promise<{
    ...
    try {
      ...
      // Group by model
      const modelGroups = recentPredictions.reduce((acc, pred) => {
  ...
  private static calculatePredictionError(actual: any, predicted: any, probability: number): number {
    if (typeof actual === 'number' && typeof predicted === 'number') {
  ...
  private static calculateMetricsByConfidence(predictions: any[], sport: string, league: string): Record<string, PredictionMetrics> {
    const bins = {
  ...
  private static calculateMetricsBySeason(predictions: any[], sport: string, league: string): Record<string, PredictionMetrics> {
    const seasons = predictions.reduce((acc, pred) => {
  ...
  private static calculateMetricsByGameType(predictions: any[], sport: string, league: string): Record<string, PredictionMetrics> {
    const gameTypes = predictions.reduce((acc, pred) => {
  ...
  private static calculateTrends(predictions: any[]): ModelPerformanceAnalysis['trends'] {
    ...
    for (let i = windowSize - 1; i < sortedPredictions.length; i++) {
      ...
      const profitability = window.reduce((sum, p) => 
        sum + this.calculateProfitLoss(p.is_correct, p.confidence), 0
      ) / window.length
  ...
  private static generateRecommendations(metrics: PredictionMetrics, trends: ModelPerformanceAnalysis['trends']): string[] {
    const recommendations: string[] = []
    
    if (metrics.accuracy < 0.55) {
      recommendations.push('Model accuracy is below expected threshold. Consider retraining with more recent data.')
    }
    
    if (metrics.profitability < -0.05) {
      recommendations.push('Model is showing negative profitability. Review betting strategy and odds calculation.')
    }
    
    if (trends.recentForm === 'declining') {
      recommendations.push('Recent performance is declining. Consider updating model features or retraining.')
    }
    
    if (metrics.brierScore > 0.25) {
      recommendations.push('High Brier score indicates poor probability calibration. Consider calibration adjustment.')
    }
  ...
  private static calculateCalibrationScore(calibrationCurve: { predictedProb: number; actualProb: number; count: number }[]): number {
  ...
  private static generateCalibrationRecommendations(score: number, curve: any[]): string[] {
    ...
    if (score > 0.1) {
      recommendations.push('Model is poorly calibrated. Consider Platt scaling or isotonic regression.')
    }
  ...
  private static async updateModelMetrics(sport: string, league: string, modelName: string): Promise<void> {
