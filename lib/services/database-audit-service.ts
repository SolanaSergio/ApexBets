...
export class DatabaseAuditService {
  ...
  async runFullAudit(): Promise<DatabaseAuditReport> {
    ...
    try {
      ...
      structuredLogger.error('Database audit failed', {
  ...
  private async testDataIntegrity(): Promise<AuditResult> {
    ...
    try {
      ...
      if (true) {
        ...
        const teamsNullSport = await supabase
          .from('teams')
        ...
        const playersNullSport = await supabase
          .from('players')
          .select('*', { count: 'exact', head: true })
          .is('sport', null)
        addIf((playersNullSport.count || 0) > 0, 'players.sport IS NULL')

        const oddsNullGameId = await supabase
          .from('odds')
          .select('*', { count: 'exact', head: true })
          .is('game_id', null)
  ...
  private async testPerformance(): Promise<AuditResult> {
    ...
    try {
      ...
      await timed(async () => {
        await supabase.from('odds').select('*', { count: 'exact', head: true })
      })
      ...
      return {
        ...
        message:
          slowQueries === 0 ? 'Performance tests passed' : `Found ${slowQueries} slow queries`,
  ...
  private async testBackupIntegrity(): Promise<AuditResult> {
    ...
    try {
      ...
      const hasData =
        dataCheck.success && dataCheck.data && dataCheck.data[0] && dataCheck.data[0].count > 0
  ...
  private async generateRecommendations(
    ...
    if (failedTests.length > 0) {
      recommendations.push(
        `Address ${failedTests.length} failed tests: ${failedTests.map(t => t.testName).join(', ')}`
      )
    }
    ...
    if (warnings.length > 0) {
      recommendations.push(
        `Review ${warnings.length} warnings: ${warnings.map(t => t.testName).join(', ')}`
      )
    }
    ...
    if (performance.tableSizes.length > 0) {
      ...
      if (largeTables.length > 0) {
        recommendations.push(
          `Consider archiving old data from large tables: ${largeTables.map(t => t.table).join(', ')}`
        )
      }
    }
      ...
      if (indexTest && indexTest.details?.missingIndexes?.length > 0) {
        for (const index of indexTest.details.missingIndexes) {
          try {
            ...
            errors.push(
              `Failed to create index ${index}: ${error instanceof Error ? error.message : String(error)}`
            )
          }
        }
      }
      ...
      if (integrityTest && integrityTest.details?.issues?.length > 0) {
        for (const issue of integrityTest.details.issues) {
          try {
            ...
            errors.push(
              `Failed to fix data integrity issue: ${error instanceof Error ? error.message : String(error)}`
            )
          }
        }
      }
  ...
  private async createIndex(indexName: string): Promise<void> {
    const indexDefinitions: { [key: string]: string } = {
      ...
      idx_odds_sport_game_timestamp:
        'CREATE INDEX IF NOT EXISTS idx_odds_sport_game_timestamp ON odds(sport, game_id, timestamp)',
      idx_predictions_sport_game_created:
        'CREATE INDEX IF NOT EXISTS idx_predictions_sport_game_created ON predictions(sport, game_id, created_at)',
      idx_players_team_sport:
        'CREATE INDEX IF NOT EXISTS idx_players_team_sport ON players(team_id, sport)',
      idx_standings_sport_league_season:
        'CREATE INDEX IF NOT EXISTS idx_standings_sport_league_season ON standings(sport, league, season)',
      idx_news_sport_published:
        'CREATE INDEX IF NOT EXISTS idx_news_sport_published ON sports_news(sport, published_at)',
      idx_value_bets_sport_active:
        'CREATE INDEX IF NOT EXISTS idx_value_bets_sport_active ON value_betting_opportunities(sport, expires_at)',
    }
