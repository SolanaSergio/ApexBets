/**
 * ENHANCED DYNAMIC TEAM LOGO SERVICE
 * Database-first approach with fuzzy matching and intelligent fallbacks
 */

import { createClient } from '@/lib/supabase/server'
import { getTeamLogoUrl as getApiLogoUrl, getFallbackImageUrl, type SportsLeague, type TeamLogoConfig } from './image-service'
...
export class DynamicTeamLogoService {
  private static instance: DynamicTeamLogoService

  private constructor() {}

  static getInstance(): DynamicTeamLogoService {
    if (!DynamicTeamLogoService.instance) {
      DynamicTeamLogoService.instance = new DynamicTeamLogoService()
    }
    return DynamicTeamLogoService.instance
  }
  ...
  async getTeamLogo(
    ...
    try {
      ...
      // 3. Generate dynamic logo (your existing fallback)
...
export async function getTeamLogoUrl(
  ...
  const service = DynamicTeamLogoService.getInstance()
...
export const dynamicTeamService = DynamicTeamLogoService.getInstance()
