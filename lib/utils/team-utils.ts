...
import {
  getTeamLogoUrl as getTeamLogoFromService,
  getPlayerPhotoUrl as getPlayerPhotoFromService,
...
const getTeamColorsFromAPI = async (
  teamName: string,
  sport: string
): Promise<{ primary: string; secondary: string } | null> => {
  try {
    ...
    const primaryColor = team.strTeamBadge?.includes('primary')
      ? extractColorFromUrl(team.strTeamBadge)
      : null
    const secondaryColor = team.strTeamLogo?.includes('secondary')
      ? extractColorFromUrl(team.strTeamLogo)
      : null
