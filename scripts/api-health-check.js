  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.data && Array.isArray(data.data)) {
        ...
        console.log(`   📊 Live: ${data.live?.length || 0}, Recent: ${data.recent?.length || 0}, Upcoming: ${data.upcoming?.length || 0}`);
...
async function main() {
  ...
  for (const sport of sports) {
    ...
    testResults.push({ endpoint: `/api/games?sport=${sport}&external=true`, ...externalGamesResult });
    ...
    // Test teams
    const teamsResult = await testAPIEndpoint(
      `/api/teams?sport=${sport}&limit=5`, 
      `${sport} teams`
    );
  ...
  const emptyResults = successful.filter(r => {
    ...
    if (r.data?.live !== undefined || r.data?.recent !== undefined) {
      return (r.data.live?.length || 0) + (r.data.recent?.length || 0) + (r.data.upcoming?.length || 0) === 0;
    }
    return false;
  });
  ...
  const dbDataEndpoints = testResults.filter(r => !r.endpoint.includes('real=true') && !r.endpoint.includes('external=true') && r.success);
  ...
  if (realDataEndpoints.length > dbDataEndpoints.length) {
    console.log('   4. Database appears to have stale/incorrect data - consider running data population scripts');
  }
  
  console.log('\n🏁 Health check complete!');
  
  // Exit with error code if critical issues found
  if (failed.length > testResults.length * 0.3) { // More than 30% failed
    process.exit(1);
  }
}
