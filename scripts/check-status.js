...
async function checkStatus() {
  try {
    ...
    // Check teams
    const { data: teams, error: teamsError } = await supabase.from('teams').select('count').limit(1)
    ...
    const [teamsCount, gamesCount, playerStatsCount, oddsCount, predictionsCount] =
      await Promise.all([
