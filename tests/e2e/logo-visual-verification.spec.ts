...
test.describe('Logo Visual Verification', () => {
  ...
  test('players page displays player photos', async ({ page }) => {
    ...
    const playerPhotos = page.locator(
      '[data-testid="player-photo"], img[alt*="player"], img[alt*="Player"]'
    )
  ...
  test('team logos load without errors', async ({ page }) => {
    ...
    const imageErrors = consoleErrors.filter(
      error => error.includes('logo') || error.includes('image') || error.includes('src')
    )
