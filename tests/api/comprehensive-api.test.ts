      ...
      it('should fetch individual team details', async () => {
        for (const sport of TEST_DATA.sports) {
          ...
          for (const teamName of teams) {
            const response = await apiRequest(`/api/teams?name=${encodeURIComponent(teamName)}&sport=${sport}`)
      ...
      it('should fetch individual player details', async () => {
        for (const sport of TEST_DATA.sports) {
          ...
          for (const playerName of players) {
            const response = await apiRequest(`/api/players?name=${encodeURIComponent(playerName)}&sport=${sport}`)
