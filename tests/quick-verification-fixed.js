...
/**
 * Quick Verification Script - Fixed Version
 * Fast check of critical ApexBets functionality with proper timeout handling
 */
...
async function runQuickVerification() {
  log(`${colors.bright}${colors.cyan}🚀 ApexBets Quick Verification (Fixed)${colors.reset}`);
  log(`${colors.cyan}============================================${colors.reset}\n`);
  
  const results = [];
  
  // Test critical API endpoints with shorter timeouts
  log(`${colors.bright}Testing API Endpoints:${colors.reset}`);
  results.push(await testApiEndpoint('/api/health', 'Health Check', 3000));
  results.push(await testApiEndpoint('/api/games?limit=5', 'Games Endpoint', 5000));
  results.push(await testApiEndpoint('/api/teams?limit=5', 'Teams Endpoint', 5000));
  results.push(await testApiEndpoint('/api/live-scores', 'Live Scores', 5000));
  results.push(await testApiEndpoint('/api/odds', 'Odds', 5000));
  results.push(await testApiEndpoint('/api/predictions', 'Predictions', 5000));
  results.push(await testApiEndpoint('/api/analytics/stats', 'Analytics', 5000));
  results.push(await testApiEndpoint('/api/standings', 'Standings', 5000));
  results.push(await testApiEndpoint('/api/value-bets', 'Value Bets', 3000));
  ...
  results.push(await testExternalApiEndpoint('https://www.balldontlie.io/api/v1/teams', 'BallDontLie API', 5000));
