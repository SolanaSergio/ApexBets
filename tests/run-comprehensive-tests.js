...
/**
 * Comprehensive Test Runner for ApexBets
 * Runs all tests with real NBA data validation
...
function runUnitTests() {
  ...
  const result = runCommand('npm run test -- --testPathPattern=unit', 'Unit tests');
  return result;
}
...
function runComprehensiveAPITests() {
  log(`${colors.bright}Running Comprehensive API Tests...${colors.reset}`);
  
  const result = runCommand('npm run test -- --testPathPattern=comprehensive', 'Comprehensive API tests');
  return result;
}
...
function generateTestReport(results) {
  log(`${colors.bright}Generating Comprehensive Test Report...${colors.reset}`);
  ...
  const report = {
    ...
    testCategories: {
      ...
      comprehensive: results.filter(r => r.name.includes('Comprehensive')).length,
  ...
  log(`${colors.green}✓ Comprehensive test report saved to: ${reportPath}${colors.reset}`);
  
  // Print summary
  log(`\n${colors.bright}Comprehensive Test Summary:${colors.reset}`);
  ...
  log(`${colors.cyan}Comprehensive API Tests: ${report.testCategories.comprehensive}${colors.reset}`);
...
function main() {
  log(`${colors.bright}${colors.cyan}🚀 Starting Comprehensive ApexBets Test Suite${colors.reset}`);
  log(`${colors.cyan}================================================${colors.reset}`);
  log(`${colors.yellow}Testing with REAL NBA data - NO MOCK DATA${colors.reset}`);
  ...
  try {
    ...
    const comprehensiveResult = runComprehensiveAPITests();
    results.push({ name: 'Comprehensive API Tests', ...comprehensiveResult, duration: Date.now() - startTime });
    ...
    if (report.summary.failed > 0) {
      ...
      log(`${colors.green}✅ All tests passed! ApexBets is working correctly with real NBA data${colors.reset}`);
      process.exit(0);
    }
