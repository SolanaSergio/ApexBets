...
async function calculateTeamStats(sport: string, team: any, games: any[], players: any[]) {
  ...
  try {
    ...
    if (sportConfig?.scoringFields) {
      return calculateSportSpecificStats(
        team,
        completedGames,
        players,
        wins,
 ...
 */
function calculateSportSpecificStats(
  team: any,
  games: any[],
  _players: any[],
  wins: number,
  ...
  return [
    ...
    { category: `${againstField} Per Game`, value: avgAgainst.toFixed(1), rank: 1, trend: 'down' },
    {
      ...
      value: differential.toFixed(1),
      rank: 1,
      trend: differential > 0 ? 'up' : 'down',
    },
  ]
}

function calculateGenericStats(
  _team: any,
  games: any[],
  players: any[],
  wins: number,
