...
function processTeamTrends(gamesData: any[], team: string, sport: string) {
  ...
  if (totalGames.length > 0) {
    ...
    trends.push({
      ...
      trend: Math.abs(parseFloat(totalVariance)) < 5 ? "neutral" : parseFloat(totalVariance) > 0 ? "up" : "down",
      value: `${totalVariance}%`,
  ...
  if (recentGames.length >= 5) {
    const recentWins = recentGames.filter(game => {
      ...
      return (isHome && game.home_score > game.away_score) || (!isHome && game.away_score > game.home_score)
    }).length
  ...
  if (divisionalGames.length > 0) {
    ...
    trends.push({
      ...
      trend: Math.abs(parseFloat(divPerformance)) < 5 ? "neutral" : parseFloat(divPerformance) > 0 ? "up" : "down",
...
function calculateStreaks(gamesData: any[], team: string) {
  ...
  for (const game of sortedGames) {
    if (team !== "all") {
      const isHome = game.home_team?.name?.toLowerCase().includes(team.toLowerCase())
      const won = (isHome && game.home_score > game.away_score) || (!isHome && game.away_score > game.home_score)
