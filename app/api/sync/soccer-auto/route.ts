...
export async function POST(request: NextRequest) {
  try {
    console.log('Starting automatic Soccer data sync...')
    
    // Fetch Soccer games and teams from ESPN API
    const today = new Date().toISOString().split('T')[0]
    
    // Get today's games from ESPN Soccer API (MLS)
    const gamesResponse = await fetch(`http://site.api.espn.com/apis/site/v2/sports/soccer/usa.1/scoreboard?dates=${today}`, {
      headers: {
    ...
    if (gamesResponse.ok) {
      ...
      console.warn(`ESPN Soccer Games API returned ${gamesResponse.status} - likely no games today`)
    }
    
    // Get teams from ESPN Soccer API (MLS)
    const teamsResponse = await fetch('http://site.api.espn.com/apis/site/v2/sports/soccer/usa.1/teams', {
      headers: {
        'Accept': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      }
    ...
    if (!teamsResponse.ok) {
      throw new Error(`ESPN Soccer Teams API error: ${teamsResponse.status}`)
    }
    ...
    console.log(`Found ${games.length} Soccer games for today`)
    console.log(`Found ${teams.length} Soccer teams`)
    ...
    if (games.length > 0) {
      ...
      for (const game of games) {
        try {
          ...
          const statusType = game.status?.type?.state
          if (statusType === 'post') {
          ...
          const { error } = await supabase
            ...
            .upsert({
              sport: 'soccer',
              league: 'MLS',
              season: '2024', // Current MLS season
    ...
    console.error('Soccer auto-sync error:', error)
    return NextResponse.json({
      success: false,
      message: 'Soccer data sync failed',
