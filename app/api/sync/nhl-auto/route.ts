...
export async function POST(_request: NextRequest) {
  try {
    ...
    // Fetch NHL games and teams from ESPN API (more reliable than NHL Stats API)
    const today = new Date().toISOString().split('T')[0]
    
    // Get today's games from ESPN NHL API
    const gamesResponse = await fetch(`http://site.api.espn.com/apis/site/v2/sports/hockey/nhl/scoreboard?dates=${today}`, {
      headers: {
    ...
    if (gamesResponse.ok) {
      ...
      console.warn(`ESPN NHL Games API returned ${gamesResponse.status} - likely no games today (off-season)`)
    }
    
    // Get teams from ESPN NHL API
    const teamsResponse = await fetch('http://site.api.espn.com/apis/site/v2/sports/hockey/nhl/teams', {
      headers: {
        'Accept': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      }
    ...
    console.log(`Found ${games.length} NHL games for today`)
        ...
        try {
          ...
          const { error } = await supabase
            ...
            .upsert({
              ...
              country: 'USA', // Most NHL teams are in USA/Canada
        ...
        try {
          ...
          const { error } = await supabase
            ...
            .upsert({
              sport: 'hockey',
              league: 'NHL',
              season: game.season?.toString() || '2024-25',
    ...
    return NextResponse.json({
      success: true,
      message: 'NHL data sync completed successfully',
      stats: {
        games: gamesSynced,
        teams: teamsSynced
      }
    ...
    console.error('NHL auto-sync error:', error)
    return NextResponse.json({
      success: false,
      message: 'NHL data sync failed',
