...
export async function POST(request: NextRequest) {
  try {
    console.log('Starting automatic NBA data sync...')
    
    // Fetch NBA games and teams from ESPN API (more reliable than NBA Stats API)
    const today = new Date().toISOString().split('T')[0]
    
    // Get today's games from ESPN API
    const gamesResponse = await fetch(`http://site.api.espn.com/apis/site/v2/sports/basketball/nba/scoreboard?dates=${today}`, {
      headers: {
    ...
    if (gamesResponse.ok) {
      ...
      console.warn(`ESPN NBA Games API returned ${gamesResponse.status} - likely no games today (off-season)`)
    }
    
    // Get teams from ESPN API
    const teamsResponse = await fetch('http://site.api.espn.com/apis/site/v2/sports/basketball/nba/teams', {
      headers: {
        'Accept': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
      }
    ...
    if (!teamsResponse.ok) {
      throw new Error(`ESPN NBA Teams API error: ${teamsResponse.status}`)
    }
    ...
    console.error('NBA auto-sync error:', error)
    return NextResponse.json({
      success: false,
      message: 'NBA data sync failed',
